use crate::tasks::pkg::install;
use flag_rs::{Command, CommandBuilder, Flag, FlagType, FlagValue};

pub fn init() -> Command {
    CommandBuilder::new("install")
            .short("Install package")
            .long(
                "Downloads and installs one or more specified software packages along with their required dependencies. Requires administrator privileges."
            )
            .flag(
                Flag::new("flathub")
            ...
            let result = install(&arguments, source_name);
            match result {
                Ok(_) => {},
